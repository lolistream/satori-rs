...
use satori_font::ParsedFont;
use satori_text::{layout_text, TextStyle};
use yoga::{
...
pub fn apply_style(n: &NodeRef, s: &ComputedStyle) {
    ...
    let is_static = matches!(s.position, None | Some(satori_css::Position::Static));
    if matches!(s.position, Some(satori_css::Position::Absolute)) {
