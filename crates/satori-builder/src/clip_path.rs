...
use crate::xml::{build_xml, js_number_to_string_f64, AttrValue};
...
pub fn parse_shape(raw: &str, width: f32, height: f32, font_size: f32) -> Option<Shape> {
    ...
    if let Some(inner) = raw.strip_prefix("inset(").and_then(|s| s.strip_suffix(')')) {
        ...
        if !radius_str.is_empty() && radius_str != "0" {
            ...
            use satori_css::style::{ComputedStyle, RadiusLen, RadiusValue};
            ...
            let d = crate::border_radius::radius_path(x as f32, y as f32, w as f32, h as f32, &synthetic);
            if !d.is_empty() {
