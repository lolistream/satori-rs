...
/// Root of this crate (`crates/satori-tests`), used as the base for all
...
pub fn init_fonts() -> Vec<satori_font::FontDescriptor> {
    let data = std::fs::read(asset("Roboto-Regular.ttf")).expect("Roboto-Regular.ttf");
    vec![satori_font::FontDescriptor {
        name: "Roboto".to_string(),
        data,
        weight: Some(400),
        style: Some(satori_font::FontStyle::Normal),
        lang: None,
    }]
}

/// One element of a `fixture.options.fonts` array. The actual font bytes live in
/// `crates/satori-tests/assets/<assetFile>` and are loaded by `load_fixture_fonts`.
...
pub struct FixtureOptions {
    ...
    /// `crates/satori-tests/assets/` (each named after the CJK / Greek
...
/// preload every non-`.ttf` file under `crates/satori-tests/assets/`**
...
/// preload here.
fn preload_callback_approximation_fonts(fonts: &mut Vec<satori_font::FontDescriptor>) {
    ...
    for entry in entries.flatten() {
        ...
        let lang = name
            ...
            .unwrap_or_default();
        fonts.push(satori_font::FontDescriptor {
...
impl Fixture {
    pub fn to_satori_options(&self) -> satori::SatoriOptions {
        let fonts: Vec<satori_font::FontDescriptor> = if self.options.fonts.is_empty() {
            init_fonts()
        } else {
            self.options.fonts.iter().filter_map(|f| {
                ...
                Some(satori_font::FontDescriptor {
                    ...
                    style: match f.style.as_deref() {
                        Some("italic") => Some(satori_font::FontStyle::Italic),
                        _ => Some(satori_font::FontStyle::Normal),
                    },
                    lang: f.lang.clone(),
                })
            }).collect()
...
pub fn to_image(svg: &str, width: u32) -> Vec<u8> {
    ...
    let playfair = std::fs::read(asset("playfair-display.ttf"))
        .expect("playfair-display.ttf in crates/satori-tests/assets");
