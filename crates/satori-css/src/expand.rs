...
use crate::dimension::{parse_dimension, Dim};
use crate::gradient::{parse_conic_gradient, parse_linear_gradient, parse_radial_gradient};
use crate::style::*;
use crate::variables::{extract_vars, merge_vars, substitute, Vars};
...
fn apply_tag_preset(tag: &str, s: &mut ComputedStyle) {
    ...
    match tag {
        "p" => {
            s.margin_top = Some(crate::dimension::Dim::Px(inherited_fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(inherited_fs));
        }
        "blockquote" => {
            s.margin_top = Some(crate::dimension::Dim::Px(inherited_fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(inherited_fs));
            s.margin_left = Some(crate::dimension::Dim::Px(40.0));
            s.margin_right = Some(crate::dimension::Dim::Px(40.0));
        }
        "center" => {
        ...
        "h1" => {
            ...
            s.margin_top = Some(crate::dimension::Dim::Px(0.67 * fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(0.67 * fs));
            s.margin_left = Some(crate::dimension::Dim::Px(0.0));
            s.margin_right = Some(crate::dimension::Dim::Px(0.0));
        }
        "h2" => {
            ...
            s.font_weight = Some(700);
            s.margin_top = Some(crate::dimension::Dim::Px(0.83 * fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(0.83 * fs));
            s.margin_left = Some(crate::dimension::Dim::Px(0.0));
            s.margin_right = Some(crate::dimension::Dim::Px(0.0));
        }
        "h3" => {
            ...
            s.font_weight = Some(700);
            s.margin_top = Some(crate::dimension::Dim::Px(fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(fs));
            s.margin_left = Some(crate::dimension::Dim::Px(0.0));
            s.margin_right = Some(crate::dimension::Dim::Px(0.0));
        }
        "h4" => {
            let fs = inherited_fs;
            s.font_weight = Some(700);
            s.margin_top = Some(crate::dimension::Dim::Px(1.33 * fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(1.33 * fs));
            s.margin_left = Some(crate::dimension::Dim::Px(0.0));
            s.margin_right = Some(crate::dimension::Dim::Px(0.0));
        }
        "h5" => {
            ...
            s.font_weight = Some(700);
            s.margin_top = Some(crate::dimension::Dim::Px(1.67 * fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(1.67 * fs));
            s.margin_left = Some(crate::dimension::Dim::Px(0.0));
            s.margin_right = Some(crate::dimension::Dim::Px(0.0));
        }
        "h6" => {
            ...
            s.font_weight = Some(700);
            s.margin_top = Some(crate::dimension::Dim::Px(2.33 * fs));
            s.margin_bottom = Some(crate::dimension::Dim::Px(2.33 * fs));
            s.margin_left = Some(crate::dimension::Dim::Px(0.0));
            s.margin_right = Some(crate::dimension::Dim::Px(0.0));
        }
        "u" => {
    ...
    match key {
        ...
        "boxSizing" => {
            s.box_sizing = match v.as_str() {
                Some("content-box") => Some(crate::style::BoxSizing::ContentBox),
                Some("border-box") => Some(crate::style::BoxSizing::BorderBox),
                _ => s.box_sizing,
            };
        }
...
fn apply_flex_shorthand(s: &mut ComputedStyle, v: &serde_json::Value, fs: f32) {
    if let Some(n) = v.as_f64() {
        ...
        s.flex_basis = Some(crate::dimension::Dim::Px(0.0));
        return;
    }
    ...
    if trimmed.eq_ignore_ascii_case("none") {
        ...
        s.flex_basis = Some(crate::dimension::Dim::Auto);
        return;
    }
    ...
    let parse_basis = |t: &str| -> Option<crate::dimension::Dim> {
        parse_dimension(
...
fn apply_background_image(s: &mut ComputedStyle, v: &serde_json::Value) {
    ...
    let pieces: Vec<String> = split_top_level_commas(raw)
        .into_iter()
        .map(|p| crate::gradient::normalize_webkit_gradient(&p))
        .collect();
