...
use satori_builder::build_decoration;
use satori_builder::text_decoration::{DecorationArgs, GlyphBox as BoxBox};
use satori_css::{
    dimension::Dim,
...
use satori_font::{FontLoader, FontStyle, ParsedFont};
use satori_handler::image::{resolve_image_asset_file, resolve_image_src, ResolvedImage};
use satori_layout::{install_text_measure, lay_out, make_yoga, Built, TextMeasureCtx};
use satori_text::{
...
pub use satori_css::style as css;
pub use satori_jsx;
...
pub struct SatoriOptions {
    ...
    pub fonts: Vec<satori_font::FontDescriptor>,
    pub embed_font: bool,
    pub debug: bool,
...
fn resolve_line_limit(parent: &ComputedStyle) -> (u32, String) {
    use satori_css::style::{Display as Disp, Overflow, TextOverflow};
...
fn render_text_node(
    ...
    let mut decoration_glyphs: std::collections::BTreeMap<usize, Vec<satori_font::GlyphBox>> =
        std::collections::BTreeMap::new();
    ...
    for (idx, word) in tl.words.iter().enumerate() {
        ...
        let baseline_of_word_f64: f64 = {
            ...
            let font = satori_font::FontLoader::resolve_for_char(&node.fonts, first_ch)
                .unwrap_or_else(|| std::sync::Arc::clone(&node.fonts[0]));
            let (_lh, base) = satori_text::line_metrics_f64(
                &font,
                tx_style.font_size,
        ...
        if embed_font && !node.fonts.is_empty() {
            ...
            if !cleaned.is_empty() {
                ...
                if should_collect_boxes {
                    ...
                    let band = satori_font::SkipInkBand {
                        underline_y,
    ...
    let text_shadow_filter = parent_style
        ...
        .filter(|v| !v.is_empty())
        .map(|shadows| {
            satori_builder::build_drop_shadow(
                &node.id,
                tl.width,
                tl.height,
                shadows,
...
fn build_node(
    ...
    let has_overflow_hidden = matches!(
        computed.overflow,
        Some(satori_css::style::Overflow::Hidden)
    );
    ...
    if tag == "div" {
        ...
        if needs_display
            && !matches!(
                computed.display,
                Some(satori_css::style::Display::Flex)
                    | Some(satori_css::style::Display::Contents)
                    | Some(satori_css::style::Display::None)
                    | Some(satori_css::style::Display::WebkitBox)
            )
        {
            return Err(SatoriError::Parse(
...
fn resolve_layers_urls(
    layers: Option<&mut Vec<satori_css::style::BackgroundImage>>,
    asset_root: &std::path::Path,
) {
    let Some(layers) = layers else { return };
    for layer in layers.iter_mut() {
        if let satori_css::style::BackgroundImage::Url { src, resolved } = layer {
            if resolved.is_some() {
                continue;
            }
            ...
            *resolved = resolved_image.map(|r| satori_css::style::ResolvedUrlImage {
                src: r.src,
...
fn parse_dimension_passthrough(
    ...
    satori_css::dimension::parse_dimension(v, fs, vw, vh)
}
