...
impl ParsedFont {
    ...
    pub fn glyph_path_d(&self, ch: char, x: f64, y: f64, font_size: f64, dp: u32) -> Option<String> {
        ...
        if let Some(contours) = super::ttf_outline::get_simple_contours(&face, gid) {
            let mut out = String::new();
            super::ttf_outline::emit_simple_path(&contours, x, y, scale, dp, &mut out);
            return Some(out);
        }
        let mut b = super::path_d::PathDataBuilder::new(x, y, scale, dp);
    ...
    pub fn run_path_d_with_fallback(
        ...
        for ch in text.chars() {
            ...
            if let Some(gid) = face.glyph_index(ch) {
                ...
                if let Some(contours) = super::ttf_outline::get_simple_contours(&face, gid) {
                    super::ttf_outline::emit_simple_path(&contours, pen_x, y, scale, dp, &mut out);
                } else {
                    let mut b = super::path_d::PathDataBuilder::new(pen_x, y, scale, dp);
                    let _ = face.outline_glyph(gid, &mut b);
                    out.push_str(&b.out);
                }
            ...
            if let Some(contours) = super::ttf_outline::get_simple_contours(&fb_face, fb_gid) {
                super::ttf_outline::emit_simple_path(&contours, pen_x, y, fb_scale, dp, &mut out);
            } else {
                let mut b = super::path_d::PathDataBuilder::new(pen_x, y, fb_scale, dp);
...
fn flatten_simple_contours(
    contours: &[Vec<super::ttf_outline::Point>],
    pen_x: f64,
    baseline_y: f64,
    scale: f64,
) -> Vec<(f64, f64, f64, f64)> {
...
mod path_tests {
    ...
    //! suite in `./` covers pixel-level parity end
    //! to end.
    use super::*;
